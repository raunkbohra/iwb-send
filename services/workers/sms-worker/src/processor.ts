...
export async function processSmsJob(
  ...
  if (!routeResult) {
    childLogger.error('No route available for SMS', {
      messageId: job.messageId,
      to: message.to,
    ...
    await prisma.message.update({
      ...
      data: {
        ...
        failureReason: 'No SMS provider available for recipient',
      },
    });
  ...
  try {
    ...
    childLogger.info('SMS sent via provider', {
      messageId: job.messageId,
      provider: routeResult.provider,
  ...
  await prisma.walletLedgerEntry.create({
    data: {
      ...
      description: `SMS sent via ${routeResult.provider}`,
  ...
  childLogger.info('SMS job fully processed', {
    messageId: job.messageId,
    provider: routeResult.provider,
